...
INSERT INTO task(title, description, created, updated, due_date, status_id, user_id) 
	VALUES('Complete homework', 'Learn about data models & relationships', '2021-01-11 16:00:00', '2021-01-17 16:00:00', '2021-01-23 00:00:00', 2, 11);
UPDATE task SET title = 'Finish homework & upload' WHERE id = 36;
UPDATE task SET due_date = '2021-01-24 12:00:00' WHERE id = 36;
UPDATE task SET status_id = 1 WHERE id = 36;
UPDATE task SET status_id = 3 WHERE id = 36;
DELETE FROM task WHERE id = 36;
SELECT title AS tasks FROM task WHERE user_id IN (SELECT id FROM user WHERE email LIKE '%@spotify.com');
SELECT * FROM task WHERE user_id IN (SELECT id FROM user WHERE name = 'Donald Duck') 
AND status_id IN (SELECT id FROM status WHERE name = 'Not started');
SELECT title FROM task WHERE created LIKE '%-09-%' 
AND user_id IN (SELECT id FROM user WHERE name = 'Maryrose Meadows');
SELECT title, description FROM task WHERE month(created)=09 
AND user_id IN (SELECT id FROM user WHERE name = 'Maryrose Meadows');
SELECT COUNT(id) AS total_in_month FROM task GROUP BY month(created);